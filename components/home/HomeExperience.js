        ...
        <div className="grid gap-8 md:grid-cols-3">
          {items.map((item, i) => (
            <motion.div
              ...
              whileHover={{ y: -8 }}
              className="rounded-2xl border border-slate-100 bg-gradient-to-b from-slate-50 to-white p-8 shadow-sm transition-shadow hover:shadow-xl"
            >
              <div className="flex h-14 w-14 items-center justify-center rounded-2xl bg-ocean-100 text-ocean-700">
                <item.icon className="h-7 w-7" />
              </div>
              <h3 className="mt-6 font-serif text-xl font-semibold text-ocean-950">
                {item.title}
              </h3>
