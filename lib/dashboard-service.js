...
export function formatTransactionsForUI(transactions) {
  return transactions.map((t) => {
    ...
    const isCredit =
      ...
      (t.metadata?.category === "Reversal" && t.type === TRANSACTION_TYPES.TRANSFER);
    const isReversal = t.status === TRANSACTION_STATUS.REVERSED || t.metadata?.category === "Reversal";
    return {
      id: t._id.toString(),
      ...
      date: t.createdAt,
      status: t.status,
      type: isCredit && !isReversal ? "credit" : "debit",
